...
const schema = z.object({
    ...
    education : z.enum(['Intermediate','Graduate','Post Graduate'], { message: "Select a valid education level" }),
    resume : z.any().refine(file => file && file.length > 0 && (file[0].type === "application/pdf" || file[0].type === "application/msword"), { message: "Resume must be a PDF or Word document" })
})
...
const ApplyJobDrawer = ({user, job, applied = false, fetchJob}) => {
  ...
  <DrawerTrigger asChild>
    <Button variant={job?.isOpen && !applied ? "blue":"destructive"} disabled={!job?.isOpen || applied} size='lg'>
