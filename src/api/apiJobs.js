...
export async function saveJob(token,{alreadySaved},saveData) {
    ...
    if(alreadySaved){
        const {data,error} = await supabase.from('saved_jobs').delete().eq('job_id',saveData.job_id);
        ...
        const {data,error} = await supabase.from('saved_jobs').insert([saveData]).select();
...
export async function updateHiringStatus(token,{job_id},isOpen) {
    ...
    const { data, error } = await supabase.from('jobs').update({ isOpen: isOpen }).eq('id', job_id).select();
